...
class Patcher(object):
    ...
    def handle_action(self, action, tree, namespaces):
        action_type = type(action)
        method = getattr(self, '_handle_' + action_type.__name__)
        method(action, tree, namespaces)

    def _handle_DeleteNode(self, action, tree, namespaces):
        node = tree.xpath(action.node, namespaces=namespaces)[0]
        node.getparent().remove(node)

    def _handle_InsertNode(self, action, tree, namespaces):
    ...
    def _handle_InsertComment(self, action, tree, namespaces):
        target = tree.xpath(action.target, namespaces=namespaces)[0]
